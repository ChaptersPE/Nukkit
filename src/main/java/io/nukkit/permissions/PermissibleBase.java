...
public class PermissibleBase implements Permissible {
    ...
    public boolean hasPermission(String inName) {
        ...
        if (isPermissionSet(name)) {
            ...
            Permission perm = Nukkit.getServer().getPluginManager().getPermission(name);
    ...
    public void recalculatePermissions() {
        clearPermissions();
        Set<Permission> defaults = Nukkit.getServer().getPluginManager().getDefaultPermissions(isOp());
        Nukkit.getServer().getPluginManager().subscribeToDefaultPerms(isOp(), parent);

        for (Permission perm : defaults) {
            String name = perm.getName().toLowerCase();
            permissions.put(name, new PermissionAttachmentInfo(parent, name, null, true));
            Nukkit.getServer().getPluginManager().subscribeToPermission(name, parent);
    ...
    public synchronized void clearPermissions() {
        ...
        for (String name : perms) {
            Nukkit.getServer().getPluginManager().unsubscribeFromPermission(name, parent);
        }

        Nukkit.getServer().getPluginManager().unsubscribeFromDefaultPerms(false, parent);
        Nukkit.getServer().getPluginManager().unsubscribeFromDefaultPerms(true, parent);
