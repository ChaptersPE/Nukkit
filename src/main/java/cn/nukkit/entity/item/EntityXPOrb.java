    ...
    public boolean onUpdate(int currentTick) {
        ...
        if (this.isAlive()) {
            ...
            if (this.onGround && (Math.abs(this.motionX) > 0.00001 || Math.abs(this.motionZ) > 0.00001)) {
                friction = this.getLevel().getBlock(this.blockTemporalVector.setComponents(Math.floor(this.x), Math.floor(this.y - 1), Math.floor(this.z) - 1)).getFrictionFactor() * friction;
            }
