...
public class Anvil extends BaseLevelProvider {
    ...
    public AsyncTask requestChunkTask(IntVector2 pos) throws ChunkException {
        ...
        this.getLevel().chunkRequestCallback(pos, stream.getBuffer(), FullChunkDataPacket.ORDER_LAYERED);
    ...
    public void setChunk(IntVector2 pos, FullChunk chunk) {
        ...
        IntVector2 region = new IntVector2(getRegionIndexX(pos.x), getRegionIndexZ(pos.z));
        this.loadRegion(region);
    ...
    protected void loadRegion(IntVector2 pos) {
        if (!this.regions.containsKey(pos)) {
            try {
                this.regions.put(pos, new RegionLoader(this, pos.x, pos.z));
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }
